...
class users{
	...
	{		
		$query=$this->conn->query("select * from test");
	...
	{
		//echo $qus;
		 $query=$this->conn->query("select * from question where testname='$qus' order by rand() ");
	    while($row=$query->fetch_array(MYSQLI_ASSOC))		
		{			
			$this->qus[]=$row;
		}
	...
	public function answer($data)
	{
		 ...
		 $examname=0;
		 $no_answer=0;
		 $query=$this->conn->query("select id,rightanswer,testname from question where testname='".$_SESSION['cat']."'");
	    while($qust=$query->fetch_array(MYSQLI_ASSOC))		
		{			
			if($qust['rightanswer']==$_POST[$qust['id']])
			{
				 $right++;
				 $examname=($qust['testname']);
			}
