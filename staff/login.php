...
if(!empty($_POST["login"])) {
	...
	$result = mysqli_query($con,"SELECT * FROM staff WHERE empid='" . $_POST["email"] . "' and password = '".md5($_POST["password"])."' and staff_category = 'Non_Teaching' ");

	if(mysqli_num_rows($result)==1){
	$row  = mysqli_fetch_array($result);
	if(is_array($row)) {
	$_SESSION["staffid"] = $row['empid'];
